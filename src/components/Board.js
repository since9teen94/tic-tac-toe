...
const Board = (props) => {
  ...
  const handleClick = (e) => {
    ...
    if (
      [...thisDiv.classList].includes("X") ||
      [...thisDiv.classList].includes("O")
    ) {
      return;
    }
  ...
  const checkWin = () => {
    const xSpaces = [...document.getElementsByClassName("X")].map((space) =>
      parseInt(space.id)
    );
    const oSpaces = [...document.getElementsByClassName("O")].map((space) =>
      parseInt(space.id)
    );
    const won = (arr) => {
      return arr.every((x) => xSpaces.includes(x)) ||
        arr.every((o) => oSpaces.includes(o))
        ? true
        : false;
    };
